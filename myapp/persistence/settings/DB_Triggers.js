...
module.exports = {
    ...
    getTriggersKeltner: function (callback) {
        new Promise(function (resolve, reject) {
            MongoClient.connect(process.env.MONGO_SERVER_URL, {useUnifiedTopology: true}, function (err, db) {
                if (err) {
                    reject(err);
                    ...
                    var dbo = db.db(process.env.MONGO_SERVER_DATABASE);
                    dbo.collection("Trigger").find({algorithm: "Keltner"}).sort({value: 1}).toArray(function (err, result) {
                        if (err) {
                            reject(err);
                        } else {
                            db.close();
    ...
    changeTriggerStatus : function (callback, trig_id, active) {
        new Promise(function (resolve, reject) {
            if(active === true){
                active = false;
            }else{
            ...
            MongoClient.connect(process.env.MONGO_SERVER_URL, {useUnifiedTopology: true}, function(err, db) {
                if (err){
                    ...
                    var dbo = db.db(process.env.MONGO_SERVER_DATABASE);
                    var myquery = {_id:  ObjectID(trig_id)};
