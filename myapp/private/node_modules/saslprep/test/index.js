...
home('should work with liatin letters', () => {
  const str = 'user';
  expect(saslprep(str)).toEqual(str);
});

home('should work be case preserved', () => {
  const str = 'USER';
  expect(saslprep(str)).toEqual(str);
});

home('should work with high code points (> U+FFFF)', () => {
  const str = '\uD83D\uDE00';
  expect(saslprep(str, { allowUnassigned: true })).toEqual(str);
});

home('should remove `mapped to nothing` characters', () => {
  expect(saslprep('I\u00ADX')).toEqual('IX');
});

home('should replace `Non-ASCII space characters` with space', () => {
  expect(saslprep('a\u00A0b')).toEqual('a\u0020b');
});

home('should normalize as NFKC', () => {
  expect(saslprep('\u00AA')).toEqual('a');
  expect(saslprep('\u2168')).toEqual('IX');
});

home('should throws when prohibited characters', () => {
...
home('should not containt RandALCat and LCat bidi', () => {
  expect(() => saslprep('a\u06DD\u00AAb')).toThrow();
});

home('RandALCat should be first and last', () => {
  expect(() => saslprep('\u0627\u0031\u0628')).not.toThrow();
  expect(() => saslprep('\u0627\u0031')).toThrow();
});

home('should handle unassigned code points', () => {
