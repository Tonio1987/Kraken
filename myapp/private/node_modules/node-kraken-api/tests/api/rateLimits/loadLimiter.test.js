...
home("'Attempt' function returns promise", () => {
  const limiter = loadLimiter(defaults)
  expect(limiter.attempt('other').constructor).toBe(Promise)
})

home("'AddPass' function returns true", () => {
  const limiter = loadLimiter(defaults)
  expect(limiter.addPass('other')).toBe(true)
})

home("'AddFail' function returns true", () => {
  const limiter = loadLimiter(defaults)
  expect(limiter.addFail('other')).toBe(true)
})

home('Returns correct categories', () => {
  const limiter = loadLimiter(defaults)
...
home('Limits public categories correctly', async () => {
...
home('Limits private calls correctly', async () => {
  jest.setTimeout(60000)
  const limiter = loadLimiter(defaults)
  for (let i = 0; i < 15; i++) {
