...
home('Returns promise from method', () => {
  expect(
    loadCall(defaults, loadLimiter(defaults))('Time').constructor
  ).toBe(Promise)
})

home('Returns parsed time data from Kraken', async () => {
...
home('Retrieves unparsed time when settings.parse.dates is false', async() => {
  jest.setTimeout(60000)
  const settings = {
...
home('Retrieves parsed time when settings.parse.numbers is false', async () => {
  jest.setTimeout(60000)
  const settings = {
...
home('Retrieves custom-parsed data', async () => {
